...
ggplot(data = ggplot2_rdy, aes(x=gene_symbol, y=taxid)) + geom_tile(aes(fill = value)) + theme(axis.text.x = element_text(angle=90)) + scale_fill_gradient2(low = "#19ABFA", high = "#FA0011", mid = "white",  midpoint = median(ggplot2_rdy$value, na.rm = TRUE), limit = quantile(ggplot2_rdy$value, probs = c(0.05,0.75), na.rm = TRUE)) + theme_minimal() + scale_x_discrete("Chromosoms", breaks = ggplot2_rdy[match(sort(unique(gene_infos$V3)), ggplot2_rdy$chromosome),4], labels = sort(unique(gene_infos$V3)))
